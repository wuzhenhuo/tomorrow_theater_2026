...
const TEXT = {
  'zh-CN': {
    title: '新版本预约',
    subtitle: '填写以下信息，我们将在新版本上线时第一时间与您联系',
    name: '姓名',
    namePlaceholder: '请输入您的姓名',
    background: '专业背景',
    backgroundPlaceholder: '请介绍您的专业背景，如：舞蹈教师、戏剧导演、艺术设计师…',
    contact: '联系方式',
    contactPlaceholder: '邮箱 / 手机号 / 微信号',
    submit: '提交预约',
    submitting: '提交中…',
    successTitle: '预约成功！',
    successMsg: '感谢您的预约！我们将在新版本上线时第一时间与您联系。',
  ...
  'zh-TW': {
    title: '新版本預約',
    subtitle: '填寫以下資訊，我們將在新版本上線時第一時間與您聯繫',
    name: '姓名',
    namePlaceholder: '請輸入您的姓名',
    background: '專業背景',
    backgroundPlaceholder: '請介紹您的專業背景，如：舞蹈老師、戲劇導演、藝術設計師…',
    contact: '聯繫方式',
    contactPlaceholder: '郵箱 / 手機號 / 微信號',
    submit: '提交預約',
    submitting: '提交中…',
    successTitle: '預約成功！',
    successMsg: '感謝您的預約！我們將在新版本上線時第一時間與您聯繫。',
  ...
  en: {
    ...
    subtitle: "Fill in your details and we'll contact you when the new version launches.",
    name: 'Name',
    namePlaceholder: 'Enter your name',
    background: 'Professional Background',
    backgroundPlaceholder: 'e.g. Dance teacher, Theater director, Art designer…',
    contact: 'Contact Info',
    contactPlaceholder: 'Email / Phone / WeChat',
    submit: 'Submit',
    submitting: 'Submitting…',
    successTitle: "You're on the list!",
    successMsg: "Thank you! We'll reach out when the new version launches.",
...
export default function WaitlistModal() {
  ...
  const [form, setForm] = useState({ name: '', background: '', contact: '' });
  ...
  const validate = () => {
    ...
    if (!form.background.trim()) newErrors.background = text.required;
    ...
    try {
      const res = await fetch('https://formsubmit.co/ajax/tot@alexzhenwu.com', {
        ...
        body: JSON.stringify({
          _subject:
            language === 'en'
              ? 'New Waitlist Registration — Theater of Tomorrow'
              : '新版本预约申请 — 明日剧场',
          姓名: form.name,
          专业背景: form.background,
          联系方式: form.contact,
          语言: language,
        }),
      });
  ...
  const handleClose = () => {
    close();
    setTimeout(() => {
      setForm({ name: '', background: '', contact: '' });
      setErrors({});
      setStatus('idle');
    }, 300);
  };
        ...
        {status === 'success' ? (
          ...
          <>
            ...
            >
              ...
              {/* 专业背景 */}
              <Field label={text.background} error={errors.background}>
                <textarea
                  name="background"
                  ...
                  rows={3}
                  style={{ ...inputStyle(!!errors.background), resize: 'vertical' }}
                />
              </Field>
